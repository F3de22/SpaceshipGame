...
void main()
{
  ...
  vec3 cw = normalize(camtar - campos); //forward dir
  vec3 cp = vec3(sin(roll), cos(roll), 0.0);
  vec3 cu = normalize(cross(cw, cp)); //right dir
  vec3 cv = normalize(cross(cu, cw)); //up dir
  vec3 rd = normalize(xy.x * cu + xy.y * cv + 1.6 * cw); //ray from the camera, FOV = 1.6

  //light 
  vec3 light = normalize(vec3(0., 0., 0.) - campos);
  float sundot = clamp(dot(light, rd), 0.0, 1.0); //light contribute

  //render space
  //glaxy center glow
  vec3 col = glow * 1.2 * min(vec3(1.0, 1.0, 1.0), vec3(2.0, 1.0, 0.5) * pow(sundot, 100.0));
  //moon haze
  col += 0.2 * vec3(0.8, 0.9, 1.2) * pow(sundot, 8.0);
  ...
  cpos += vec3(3831.0 - timeS * 39., 221.0, 999.);
  col += 0.03 * vec3(0.6, 0.0, 0.0) * 10.0 * pow((fbmslow(cpos * 0.0145)), 2.0);

  //stars
  cpos += 1500. * rd + vec3(831., 321.0, 999.);
  col += stars * fbmslow(cpos * 0.0021);

  //clouds 
  vec2 shift = vec2(timeS * 100.0, timeS * 180.0); //move noise 
  vec4 sum = vec4(0, 0, 0, 0); //accumulator
  float c = campos.y / rd.y;
  vec3 cpos2 = campos - c * rd; //hit position
  float radius = length(cpos2.xz)/1000.0; //distance from the origin
  ...
  if(radius < 1.8)
  {
    for(int q=10; q > -10; q--) // layers
    {
      ...
      vec3 shine = mix(lightVis, lightUnvis, smoothstep(0.0, 1.0, see)); //shine the cloud area aligned to the camerapos
      
      // border
      float radius = length(cpos.xz) / 999.;
      if(radius>1.0)continue;
      //rotation velocity 
      float rot = 3.00 * (radius) - timeS; 
      ...
      //front-to-back volume blending -> simulate absorption
    ...
    for(int q=0; q < 20; q++)
    {
      ...
      float rot = 3.2 * (radius) - timeS * 1.1;
      cpos.xz = cpos.xz * mat2(cos(rot), -sin(rot), sin(rot), cos(rot));
      cpos += vec3(831.0 + shift.x, 321.0 + float(q) * mix(250.0, 50.0, radius)- shift.x * 0.2, 1330.0+ shift.y); // cloud position
